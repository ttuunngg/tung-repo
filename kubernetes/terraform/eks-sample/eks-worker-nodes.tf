...
data "aws_ami" "eks-worker" {
  filter {
    name   = "name"
    values = ["amazon-eks-node-${aws_eks_cluster.tf-eks.version}-v*"]
  }

  most_recent = true
  owners      = ["960562133150"] # Amazon EKS AMI Account ID
}
...
#!/bin/bash
set -o xtrace
/etc/eks/bootstrap.sh --apiserver-endpoint '${aws_eks_cluster.tf-eks.endpoint}' --b64-cluster-ca '${aws_eks_cluster.tf-eks.certificate_authority.0.data}' '${var.cluster-name}'
USERDATA
}

resource "aws_launch_configuration" "tf-eks" {
...
resource "aws_autoscaling_group" "tf-eks" {
  desired_capacity     = 2
  launch_configuration = "${aws_launch_configuration.tf-eks.id}"
  max_size             = 2
  min_size             = 1
  name                 = "tf-eks"
  vpc_zone_identifier  = "${aws_subnet.tf-eks[*].id}"
